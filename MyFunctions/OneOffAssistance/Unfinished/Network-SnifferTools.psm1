...
function Install-MicrosoftMessageAnalyzer {
    if (!$(Test-Path "$HOME\Downloads\MessageAnalyzer64.msi")) {
        try {
    ...
    $MSIFullPath = "$HOME\Downloads\MessageAnalyzer64.msi"
...
function Start-Sniffer {
    ...
    Param(
        ...
        [Parameter(Mandatory=$False)]
        [String]$Port="all", # Gets ports that match Source/Local AND Destination/Remote
    ...
    Write-Host "IMPORTANT NOTE: There is about a 60 second delay between network activity and output!" -fore yellow
    Write-Host ""
    $ESCKey = 27
    $running = $true
    ...
    while ($running) {
        ...
        function NoCaptureOutput {
            ...
            Write-Host "----------------------------------------------------------------------"
            Set-Content -Path "$($OutputFile | Split-Path -Parent)\Latest_Packet.txt" -Value $($(
                "Time:`t`t$(get-date)"
        ...
        function CaptureOutput {
            ...
            if ($($PSCustomObjOutput.PSObject.Properties) -ne $null) {
                ...
                Set-Content -Path "$($OutputFile | Split-Path -Parent)\Latest_Packet.txt" -Value $PSCustomObjOutput
...
<#
.SYNOPSIS
    Some of us are forced to use bad software. Sometimes this software can continue to make TCP connections
    for no reason and to the detriment business operations. For these situations, this function will watch
    for TCP communication occurring by the specified BadProgramName between the local and remote hosts,
    filtered by a combination of LocalIP, LocalPort, RemoteHostNameOrIP, and/or RemotePort parameters.

    This function determines whether or not to kill a particular TCP connection by matching a string contained
    in the data either sent or received by the Local or Remote Host. This string is defined by the admin
    running this function.

.DESCRIPTION
    See .Synopsis

    IMPORTANT NOTE 1: The goal of this function is to kill ONLY ONE SPECIFIC TCP connection. If one specific TCP
    connection cannot be determined by the combination of parameters used, then the user will receive
    an interactive prompt to choose.
    ...
    use the -PerpetualWatch parameter, which will put the function in a loop until it is killed manually (either
...
function Watch-BadProgramConnection {
    ...
    Param(
        ...
        [Parameter(Mandatory=$False)]
        [int]$PersistentTrafficTimeoutInMinutes,

        [Parameter(Mandatory=$False)]
        [int]$NoTrafficTimeoutInMinutes,
    ...
    ##### BEGIN Variable/Parameter Transforms and PreRun Prep #####
    if (!$ProblematicData -and !$PersistentTrafficTimeoutInMinutes) {
        Write-Error "You must use either -ProblematicData or -PersistentConnectionTimeoutInMinutes or both of these parameters! Halting!"
        $global:FunctionResult = "1"
        return
    }
    ...
    if ($(Get-Process).Name -notcontains $BadProgramName) {
    ...
    try {
        $global:FunctionResult = "0"
        Install-NirsoftCPorts -AddToSystemPath
    ...
    try {
        $PotentialTCPConnectionsToKill = Get-NetTCPConnection @GetNetTCPConnParams
        ...
        Write-Error "Unable to find a TCP connection that matches the parameters specified! Halting!"
        $global:FunctionResult = "1"
        return
    }
    ...
    elseif ($PotentialBadProgramProcesses.Count -lt 1) {
        Write-Error "Can't find any TCP connections based on parameters supplied to Get-TCPConnection cmdlet! Halting!"
        $global:FunctionResult = "1"
        return
    }
    ...
    if ($RemoteIP -and $RemotePort) {
        $StartSnifferArgsAsString = "-ScanIP $RemoteIP -RemotePort $RemotePort"
    }
    ...
    Write-Host "Waiting to receive network activity regarding $RemoteIP on Port $RemotePort ..."
    $LatestPacketLogFile = "$SnifferOutputFileParentDir\Latest_Packet.txt"
    while (!$(Test-Path $LatestPacketLogFile) -or 
    $($(Get-Date) - $(Get-Item $LatestPacketLogFile).LastWriteTime) -gt $(New-TimeSpan -Seconds 5)) {
        Write-Verbose "Waiting to receive network activity.."
        Start-Sleep -Seconds 1
    }
    Write-Host "Started receiving network activity ..."
    [datetime]$TimeOfLatestPacketReceived =  $(Get-Item $LatestPacketLogFile).LastWriteTime
    [datetime]$StartedReceivingPacketsStartTime = Get-Date

    # Give the network stack some time to start sending packets to our Sniffer socket
    Start-Sleep -Seconds 60

    try {
        $ReceiveJobPrep = Receive-Job -Name Sniffer
        # The first six lines of the Revceive-Job output aren't JSON, so let's just skip them...
        [System.Collections.ArrayList]$ReceiveJobJson = $ReceiveJobPrep[5..$ReceiveJobPrep.Count]
    }
    catch {
        Write-Error "The network stack did not send any packets to the Sniffer socket within 60 seconds! Halting!"
        $global:FunctionResult = "1"
        return
    }

    [System.Collections.ArrayList]$ConnectionStillExists = @("True")
    while ($ConnectionStillExists -contains "True") {
        # First, make sure there's still a TCP Connection matching $TCPConnectionToKill
        $AllTCPConns = Get-NetTCPConnection
        [System.Collections.ArrayList]$ConnectionStillExists = @()
        foreach ($conn in $AllTCPConns) {
            if ($conn.LocalAddress -eq $TCPConnectionToKill.LocalAddress -and 
            $conn.LocalPort -eq $TCPConnectionToKill.LocalPort -and 
            $conn.RemoteAddress -eq $TCPConnectionToKill.RemoteAddress -and
            $conn.RemotePort -eq $TCPConnectionToKill.RemotePort) {
                $null = $ConnectionStillExists.Add("True")
            }
            else {
                $null = $ConnectionStillExists.Add("False")
            }
        }
        if ($ConnectionStillExists -notcontains "True") {
            $ConnNoLongerExists = $true
            # Break the while loop... 
            break
        }

        $NoChangeInLatestPacketTime = $(Get-Item $LatestPacketLogFile).LastWriteTime -eq $TimeOfLatestPacketReceived
        $ChangeInLatestPacketTime = $(Get-Item $LatestPacketLogFile).LastWriteTime -ne $TimeOfLatestPacketReceived
        $ReceiveJobPrepInternal = Receive-Job -Name Sniffer
        if ($ReceiveJobPrepInternal.Count -ne 0) {
            Write-Host "Received more Packets..."
            foreach ($jsonString in $ReceiveJobPrepInternal) {
                $null = $ReceiveJobJson.Add($jsonString)
            }
            ...
            [datetime]$TimeOfLatestPacketReceived =  $(Get-Item $LatestPacketLogFile).LastWriteTime
            # If $StoppedReceivingPacketsStartTime datetime is more recent than $StartedReceivingPacketsStartTime datetime,
            # and since we just received some packets, reset $StartedReceivingPacketsStartTime datetime
            if ($StoppedReceivingPacketsStartTime -gt $StartedReceivingPacketsStartTime) {
                [datetime]$StartedReceivingPacketsStartTime = Get-Date
                $StoppedReceivingPacketsStartTime = $null
            }
        }
        else {
            if ($StartedReceivingPacketsStartTime -gt $StoppedReceivingPacketsStartTime) {
                if ($StoppedReceivingPacketsStartTime -eq $null) {
                    [datetime]$StoppedReceivingPacketsStartTime = Get-Date
                    $StartedReceivingPacketsStartTime = $null
                }
            }
        }

        $AmountOfTimeWithoutNetworkCommunication = $(Get-Date) - $StoppedReceivingPacketsStartTime
        if ($NoTrafficTimeoutInMinutes) {
            if ($AmountOfTimeWithoutNetworkCommunication -gt $(New-TimeSpan -Minutes $NoTrafficTimeoutInMinutes)) {
                Write-Host "There hasn't been any network communication on the TCP connection we are monitoring for at least $NoTrafficTimeoutInMinutes minute(s)..."
                Write-Host "Killing TCP Connection..."
                $KillTCPConnection = $true
            }
        }
        
        $AmountOfTimeWithPersistentNetworkCommunication = $(Get-Date) - $StartedReceivingPacketsStartTime
        if ($PersistentTrafficTimeoutInMinutes) {
            if ($AmountOfTimeWithPersistentNetworkCommunication -gt $(New-TimeSpan -Minutes $PersistentTrafficTimeoutInMinutes)) {
                Write-Host "There has been persistent network communication on the TCP connection we are monitoring for at least $PersistentTrafficTimeoutInMinutes minute(s)..."
                Write-Host "Killing TCP Connection..."
                $KillTCPConnection = $true
            }
        }
        ...
        if ($KillTCPConnection) {
            ...
            # $TCPConnectionToKill
            cports.exe /close $LocalIP * $RemoteIP $RemotePort $TCPConnectionToKill.OwningProcess
    ...
    if ($PerpetualWatch) {
        ...
        [System.Collections.ArrayList]$ParametersAsStringArray = @()
        foreach ($kvpair in $BoundParametersDictionary.GetEnumerator()) {
            $null = $ParametersAsStringArray.Add("-$($kvpair.Key)" + " " + "$($BoundParametersDictionary[$kvpair.Key])")
        }
        $InvokeExpressionWatchBadProgramString = $FunctionName + " " + $(ParametersAsStringArray -join " ")
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUSqE3oLeGuWEjkuOktXhl2W1z
# kWegggn9MIIEJjCCAw6gAwIBAgITawAAAB/Nnq77QGja+wAAAAAAHzANBgkqhkiG
...
# NwIBCzEOMAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFIU9f61uij1nbMgW
# JRd7hdYyOkPPMA0GCSqGSIb3DQEBAQUABIIBAALtaDbZ5tfviUm1L9O7TL1/hhpi
# iI1c4IQmokp7q6rkU2fmMZa4Z7AkK07fV+T41/sgDhkyDBSa1MBu31w3/lMP9o94
# aZ7urZ9x3XGrSgrRmigiRdgEOMpT7v50+hk8mMAQd3KkG+kb2bn4f6bifU2ntoDu
# 5GXv9Ipmhl6dloLJ9NwmTeYjlOGx/0TLtn30b+2UZiG5mcSR+HByIZt2NsEBZ5MS
# M3PVcXlWWp2Pl5gS31YygZR2HWoqDcHCF0jVpjKK1JfC6YvVxMrF7WYXurd/ZAGX
# GXv5YTUs/5TCaoRl34Sh/dIl9e+SRz657rQ3W92Nu+Ss1wAXpT70bZ2kDkk=
