...
function Nuke-Bytebot {
  ...
  if ($RepoPath -and (Test-Path $RepoPath)) {
    $composeFile = Join-Path $RepoPath "docker/docker-compose.proxy.yml"
...
$ComposeYml      = Join-Path $DockerDir 'docker-compose.proxy.yml'
...
if (Test-Path $EnvPath) {
  Rename-Item -Path $EnvPath -NewName ($EnvPath + ".bak") -Force
} else {
  New-Item -ItemType File -Path $EnvPath | Out-Null
}

[System.Collections.ArrayList]$envLines = @()
...
if ([string]::IsNullOrWhiteSpace($anthInput)) {
  $envLines.Add("ANTHROPIC_API_KEY=placeholder")
} else {
  $envLines.Add("ANTHROPIC_API_KEY=$anthInput")
}

# Prompt for OPENAI_API_KEY (optional)
$openaiInput = Read-Host "Enter your OPENAI_API_KEY (press Enter to keep existing or set later)"
if ([string]::IsNullOrWhiteSpace($openaiInput)) {
  $envLines.Add("OPENAI_API_KEY=placeholder")
} else {
  $envLines.Add("OPENAI_API_KEY=$openaiInput")
}

# Generate BYTEBOT_ENCRYPTION_KEY
$b = New-Object byte[] 32
[System.Security.Cryptography.RandomNumberGenerator]::Create().GetBytes($b)
$hex = ($b | ForEach-Object { $_.ToString("x2") }) -join ""
$envLines.Add("BYTEBOT_ENCRYPTION_KEY=$hex")
Write-Host "Generated BYTEBOT_ENCRYPTION_KEY" -ForegroundColor Green

# Write out the new .env file in UTF-8 no BOM
Write-Utf8NoBom -Path $EnvPath -Content ($envLines -join "`n")

# Update LiteLLM model_list for proxy mode
$LiteLLMConfigPath = Join-Path $repoPath "packages\bytebot-llm-proxy\litellm-config.yaml"
$ModelListYaml = @"
model_list:
  # Anthropic Models
  ...
  - model_name: gemini-2.5-flash
    litellm_params:
      ...
      api_key: os.environ/GEMINI_API_KEY
"@
Set-Content -Path $LiteLLMConfigPath -Value $ModelListYaml -Encoding UTF8

# Change default port on host for bytebot-llm-proxy to avoid conflict with NoMachine
$YamlContent = Get-Content $ComposeYml
$UpdatedYamlContent = $YamlContent -replace '      - "4000:4000"', '      - "4001:4000"'
Set-Content -Path $ComposeYml -Value $UpdatedYamlContent -Encoding UTF8

# Bring the stack down/up with --env-file and override
Push-Location $repoPath
Write-Host "docker compose down" -ForegroundColor Cyan
docker compose -f "$repoPath\docker\docker-compose.proxy.yml" --env-file $EnvPath -f $ComposeYml down

Write-Host "docker compose up -d --build (with --env-file docker\.env + override)" -ForegroundColor Cyan
docker compose -f "$repoPath\docker\docker-compose.proxy.yml" --env-file $EnvPath -f $ComposeYml up -d --build
# Patch VS Code desktop launcher to suppress WSL nag
docker compose -f "$repoPath\docker\docker-compose.proxy.yml" --env-file $EnvPath -f $ComposeYml exec -T bytebot-desktop sh -lc "sed -i 's|^Exec=.*|Exec=env DONT_PROMPT_WSL_INSTALL=1 /usr/bin/code --password-store=basic %F|' /home/user/Desktop/code.desktop"
Pop-Location
...
# Use the below only if you are not using byebot-llm-proxy
...
# 1) Check in the LLM proxy (API keys should be here)
$llmProxyContainerName = ((docker ps -a --format "{{.Names}} {{.Ports}}" | Where-Object {$_ -match 4001}) -split ' ')[0]
$llmProxyServiceName = "bytebot-llm-proxy"
docker cp $envCheckFile ($llmProxyContainerName + ':/tmp/env_check.sh')
docker compose --env-file "$EnvPath" -f $ComposeYml exec -T $llmProxyServiceName sh -lc 'sh /tmp/env_check.sh'

# 2) Check in the agent (only encryption key is expected)
$bytebotAgentContainerName = ((docker ps -a --format "{{.Names}} {{.Ports}}" | Where-Object {$_ -match 9991}) -split ' ')[0]
$bytebotAgentServiceName = "bytebot-agent"
docker cp $envCheckFile ($bytebotAgentContainerName + ':/tmp/env_check.sh')
docker compose --env-file "$EnvPath" -f $ComposeYml exec -T $bytebotAgentServiceName sh -lc 'sh /tmp/env_check.sh | grep BYTEBOT_ENCRYPTION_KEY || true'
...
#Write-Host ("  docker compose -f `"{0}\docker/docker-compose.proxy.yml`" logs -f" -f $repo) -ForegroundColor Yellow
...
docker compose -f "$repoPath\docker\docker-compose.proxy.yml" --env-file "$EnvPath" -f docker/docker-compose.proxy.yml down -v

# Rebuild and relaunch
docker compose -f "$repoPath\docker\docker-compose.proxy.yml" --env-file "$EnvPath" -f docker/docker-compose.proxy.yml up -d --build
docker compose -f "$repoPath\docker\docker-compose.proxy.yml" --env-file "$EnvPath" -f docker/docker-compose.proxy.yml logs -f bytebot-agent
"@
